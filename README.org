...
m = df.merge(g, left_index=True, right_index=True, how='outer', suffixes=['_dat','_graph'])
...
# ax.plot(df['t'], df.index, label='data')
# ax.plot(g['t'], g.index, label='graphic')
# ax.plot(m['t_dat'], m.index, label='interp')
ax.plot(m['t_dat'], m.index, label='data')
ax.plot(m['t_graph'], m.index, label='graphic')
ax.set_ylim(ax.get_ylim()[::-1])
ax.legend(loc=3)

ax2 = ax.twiny()
ax2.plot(m['t_dat'] - m['t_graph'], m.index, label='diff')
...
There is only one Agassiz borehole, but it has been measured several times. 
